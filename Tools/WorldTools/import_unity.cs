...
internal class CommandScript : IRunCommand
{
    ...
    public void Execute(ExecutionResult result)
    {
        ...
        if (File.Exists(scenePath)) throw new InvalidOperationException("Building map scene already exists; inspect before replacing it.");
        var prefab=AssetDatabase.LoadAssetAtPath<GameObject>(assetPath);
        if (!prefab) throw new InvalidOperationException("Building map prefab has not imported.");
        var scene=EditorSceneManager.NewScene(NewSceneSetup.EmptyScene,NewSceneMode.Additive);
        SceneManager.SetActiveScene(scene);
        var root=(GameObject)PrefabUtility.InstantiatePrefab(prefab,scene);
        root.name="Rockport Buildings - Original Game Layout";
