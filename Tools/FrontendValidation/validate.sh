#!/usr/bin/env bash
# Run Unity tests in an owned, isolated copy, never in the user's live project.
set -euo pipefail
...
if [[ -z "$workspace" ]]; then
  ...
  [[ "$workspace" != "$source_project" && -f "$workspace/.mw-frontend-validation-source" ]] || { echo 'Not an owned frontend validation workspace.' >&2; exit 2; }
  [[ "$(< "$workspace/.mw-frontend-validation-source")" == "$source_project" ]] || { echo 'Workspace belongs to another source project.' >&2; exit 2; }
...
result="$(mktemp -d "$source_project/Tools/FrontendValidation/Evidence/tests-$(date -u +%Y%m%dT%H%M%SZ).XXXXXX")"
...
args=(-batchmode -nographics -projectPath "$workspace" -job-worker-count 4
...
if root.get('failed', '0') != '0' or int(root.get('passed', '0')) == 0:
    raise SystemExit(1)
PY
[[ "$status" -eq 0 ]] || { echo "Unity exited $status despite XML; inspect the log." >&2; exit "$status"; }
printf 'Evidence: %s\n' "$result"
